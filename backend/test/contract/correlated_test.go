...
func TestCorrelatedResponseMatchesTheDocumentedSchema(t *testing.T) {
	spec := loadGeneratedSpec(t)
	svc := service.NewCorrelationService(stubCorrelated{record: fullRecord()}, nil, nil)
...
func TestUnknownCorrelationIDIsNotFound(t *testing.T) {
	svc := service.NewCorrelationService(stubCorrelated{err: chdata.ErrCorrelatedNotFound}, nil, nil)
